...
def train_one_epoch(model,train_loader,criterion,optimizer,device,gradient_accumulation_steps, save_steps, save_dir):
    ...
    for i, batch in enumerate(train_loader, 0):
        ...
        # 计算累积损失和样本数
        accumulated_loss += total_loss.item() * N * 2
        accumulated_exp_loss += exp_loss.item() * N * 2
        accumulated_clip_loss += clip_loss.item() * N * 2
        accumulated_samples += N*2
        ...
        if (i + 1) % gradient_accumulation_steps == 0:
            ...
            avg_exp_loss = accumulated_exp_loss/ accumulated_samples 
            avg_clip_loss = accumulated_clip_loss / accumulated_samples
...
def eval_one_epoch(model,eval_loader,criterion,device, save_dir):
    ...
    with torch.no_grad():
        for i, batch in enumerate(eval_loader, 0):
            ...
            exp_loss += loss['gene_exp_ce_loss'].item() * N * 2
            clip_loss += loss['clip_loss'].item() * N * 2
...
def train_multi_epoch(model,
    ...
    for i in range(epochs):
        print(f'epochs: {i} begin training:' + '===='*20)
        ...
        with open(os.path.join(save_dir, 'eval_result'), 'a') as f:
